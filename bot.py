...
# Токен бота (будет из GitHub Secrets или переменной окружения)
TOKEN = os.getenv('rxjxKIesTkisNlHNua9jgndy8ZgJjEDD', 'ВАШ_ТОКЕН_СЮДА_ДЛЯ_ТЕСТА')
...
async def преды(ctx, member: discord.Member):
    ...
    for i, warn in enumerate(warns[user_id][-5:], 1):  # Показываем последние 5
        embed.add_field(
...
if __name__ == "__main__":
    # Для локального теста вставь токен, для GitHub будет из переменной
    if TOKEN == 'ВАШ_ТОКЕН_СЮДА_ДЛЯ_ТЕСТА':
        print("⚠️ ВНИМАНИЕ: Не указан токен бота!")
        print("📌 Для запуска укажите токен в переменной DISCORD_TOKEN или в коде")
    else:
        bot.run(TOKEN)
